/*
https://www.codewars.com/kata/simple-fun-number-9-array-packing/
...
function arrayPacking(a) {
  return a.reverse()
    ...
    .reduce((i, j) => i + j)
    .split('')
    .map((x, i, arr) => x * 2 ** (arr.length - 1 - i))
    .reduce((acc, b) => acc + b)
}
//Alternative
arrayPacking = a => parseInt(a.reverse().map(x => ('00000000' + x.toString(2)).slice(-8)).reduce((i, j) => i + j), 2)
