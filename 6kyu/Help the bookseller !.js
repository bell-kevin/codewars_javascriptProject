...
function stockList(listOfArt, listOfCat) {
  if (listOfArt == [] || listOfCat == [])
    return ''
  var res = []
  for (var i = 0, count; i < listOfCat.length; i++) {
    count = listOfArt.filter(a => a[0] == listOfCat[i]).map(a => a.replace(/\D/g, ''))
    count = count.length != 0 ? count.reduce((acc, b) => acc + +b, 0) : 0
    res.push(`(${listOfCat[i]} : ${count})`)
  }
  if (res.join(' - ').replace(/\D/g, '').replace(/0/g, '').length == 0)
    return ''
  return res.join(' - ')
}
